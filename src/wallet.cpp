...
int64_t CWallet::GetOKCashDebit(const CTxIn& txin) const
{
    if (!txin.IsAnonInput())
        return 0;
...
int64_t CWallet::GetOKCashCredit(const CTxOut& txout) const
{
...
int64_t CWallet::GetOKCashBalance() const
{
    int64_t nTotal = 0;

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->IsTrusted() && pcoin->nVersion == ANON_TXN_VERSION)
                nTotal += pcoin->GetAvailableOKCashCredit();
        };
    }
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    // Calculate coin age reward
    {
        ...
        int64_t nReward = Params().GetProofOfStakeReward(pindex->nHeight, nCoinAge, nFees);
        if (nReward <= 0)
            return false;
